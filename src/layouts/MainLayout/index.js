...
const MainLayout = props => {
    ...
    return (
        ...
        >
            ...
            {roleCode !== null && roleCode === "CSR" && !isDesktop && !isIpad &&
                <BottomNavigation
                    ...
                    showLabels={true}
                    label="Home"
                    className={classes.rootForTab}
                >
                    <BottomNavigationAction
                        showLabel={true}
                        label="Home" onClick={() => onLink("/customer/user-home")}
                        classes={{
                            root: classes.roots,
                    ...
                    <BottomNavigationAction
                        showLabel={true}
                        label="Schedule Appointment" onClick={() => onLink("/customer/scheduleAppointment")}
                        classes={{
                            root: classes.roots,
                            selected: classes.selected
                        }} icon={<AssignmentTurnedInIcon size={32} />} />
                    <BottomNavigationAction
                        showLabel={true}
                        label="Provide Feedback" onClick={() => onLink("/customer/provide-feedback")}
                        classes={{
                            root: classes.roots,
                            selected: classes.selected
                        }} icon={<FeedbackIcon size={32} />} />
                </BottomNavigation>
            }
        </div>
    );
