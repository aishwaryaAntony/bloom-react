...
import { sendApplePassToWallet, downloadQRCode, sendCustomerQRCodeEmail } from "../../store/actions/applePass";
...
class QrCode extends Component {
    ...
    sendEmail = (userObj) => {   
        ...
        this.props.dispatch(sendCustomerQRCodeEmail(this, data));
    }

    render() {
        const { classes } = this.props;         
        // console.log(`UserQRCode ==> ${JSON.stringify(this.props.userQrCode)}`)       
        return (
            <div className={classes.root}>
                <Snackbar
                    ...
                    <Alert icon={false} variant="filled" severity="success" >QR Code sent to your registered email successfully.</Alert>
                </Snackbar>
                        ...
                        <>
                            <div style={{flexDirection: 'column', display: 'flex', alignItems: 'center'}}>
                                <div style={{flexDirection: 'row', display: 'flex', alignItems: 'center'}}>
                                    <div style={{ flex: "display", justifyContent: "center", alignItems: "center", marginRight: 5 }} onClick={() => this.walletButtonAction()}>
                                        <Image src={walletButton} width="140" height="50" />
                                    </div>
                                    <Button target="_blank" href={`${ACCOUNT_END_POINT}user/qr-code/${this.props.userQrCode !== undefined ? this.props.userQrCode.qr_code : ""}`} variant="contained" size={"medium"} style ={{backgroundColor:"#345C8C", width:150, height:49, borderRadius:10}}>
                                        <Typography fontWeight="bold" fontSize={15} textAlign="center" fontFamily="Futura-Heavy" color="white" style={{ textTransform: "capitalize" }}>Download QR</Typography>
                                    </Button>
                                </div>
                                {
                                    this.props.userQrCode.email !== null &&
                                    <Button onClick={()=> this.sendEmail(this.props.userQrCode)} variant="contained" size={"medium"} style ={{backgroundColor:"rgba(26, 137, 23, 0.7)", height:30, borderRadius:10, margin: 10 }}>
                                        <Typography fontWeight="bold" fontSize={15} textAlign="center" fontFamily="Futura-Heavy" color="white" style={{ textTransform: "capitalize" }}>Send Email</Typography>
                                    </Button>
                                }
                            </div>
                        </>
